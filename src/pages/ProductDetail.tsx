...
import { ArrowLeft, Heart, Share2, Phone, MessageCircle, Star, ChevronLeft, ChevronRight } from "lucide-react";
...
const ProductDetail = () => {
  ...
  const vehicle = {
    ...
    description: "Porsche 911 Carrera S 2024 là biểu tượng của sự hoàn hảo trong thiết kế và hiệu suất. Với động cơ twin-turbo 3.0L mạnh mẽ và thiết kế thể thao đặc trưng, đây là chiếc xe dành cho những ai đam mê tốc độ và phong cách.",
    features: [
  ...
  const reviews = [
    {
      ...
      rating: 5,
      date: "2 tháng trước",
      content: "Xe tuyệt vời! Động cơ mạnh mẽ, thiết kế đẹp. Dịch vụ tư vấn của anh Thật rất chuyên nghiệp.",
      verified: true
    },
    {
      id: 2,
      name: "Lê Thị Hương",
      avatar: "https://images.unsplash.com/photo-1494790108755-2616b612b786?ixlib=rb-4.0.3&auto=format&fit=crop&w=1974&q=80",
      rating: 5,
      date: "1 tháng trước", 
      content: "Trải nghiệm lái tuyệt vời. Hỗ trợ thủ tục rất nhanh chóng và thuận tiện.",
      verified: true
    }
  ];
      ...
      <main className="pt-32 pb-16">
        <div className="container mx-auto px-4">
          ...
          <div className="flex items-center space-x-2 mb-8 font-inter text-sm text-gray-600">
            ...
            <span>/</span>
            <Link to="/products" className="hover:text-black">Xe cao cấp</Link>
            <span>/</span>
            <span className="text-black">{vehicle.brand} {vehicle.model}</span>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 mb-16">
                ...
                {vehicle.images.map((image, index) => (
                  <button
                    ...
                    className={`w-20 h-16 rounded-lg overflow-hidden border-2 transition-colors ${
                      currentImageIndex === index ? 'border-luxury-gold' : 'border-gray-200'
                    }`}
                  >
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="font-inter text-gray-600">{vehicle.year} • {vehicle.segment}</p>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {vehicle.colors.map((color, i) => (
                    <span key={i} className="bg-gray-100 px-3 py-1 rounded-full text-sm font-inter">
                      {color}
                    </span>
                  ))}
                </div>
              ...
              <div className="space-y-4 pt-6 border-t">
                ...
                <Button size="lg" variant="outline" className="w-full font-inter text-lg py-6">
          ...
          <div className="mb-16">
            <Tabs defaultValue="specs" className="w-full">
              <TabsList className="grid w-full grid-cols-3">
                <TabsTrigger value="specs" className="font-inter">Thông số kỹ thuật</TabsTrigger>
                <TabsTrigger value="offers" className="font-inter">Ưu đãi độc quyền</TabsTrigger>
                <TabsTrigger value="reviews" className="font-inter">Đánh giá khách hàng</TabsTrigger>
              </TabsList>
                ...
                <div className="grid md:grid-cols-3 gap-8">
                  <Card>
                    <CardContent className="p-6">
                      <h3 className="font-inter text-lg font-semibold mb-4">Động cơ</h3>
                      <div className="space-y-3">
                        <div className="flex justify-between">
                          <span className="text-gray-600">Loại:</span>
                          <span className="font-medium">{vehicle.specifications.engine.type}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Dung tích:</span>
                          <span className="font-medium">{vehicle.specifications.engine.displacement}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Công suất:</span>
                          <span className="font-medium">{vehicle.specifications.engine.power}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Mô-men xoắn:</span>
                          <span className="font-medium">{vehicle.specifications.engine.torque}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-6">
                      <h3 className="font-inter text-lg font-semibold mb-4">Hiệu suất</h3>
                      <div className="space-y-3">
                        <div className="flex justify-between">
                          <span className="text-gray-600">Tăng tốc:</span>
                          <span className="font-medium">{vehicle.specifications.performance.acceleration}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Tốc độ tối đa:</span>
                          <span className="font-medium">{vehicle.specifications.performance.topSpeed}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Tiêu hao nhiên liệu:</span>
                          <span className="font-medium">{vehicle.specifications.performance.fuelConsumption}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-6">
                      <h3 className="font-inter text-lg font-semibold mb-4">Kích thước</h3>
                      <div className="space-y-3">
                        <div className="flex justify-between">
                          <span className="text-gray-600">Dài:</span>
                          <span className="font-medium">{vehicle.specifications.dimensions.length}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Rộng:</span>
                          <span className="font-medium">{vehicle.specifications.dimensions.width}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Cao:</span>
                          <span className="font-medium">{vehicle.specifications.dimensions.height}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Chiều dài cơ sở:</span>
                          <span className="font-medium">{vehicle.specifications.dimensions.wheelbase}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      {vehicle.exclusiveOffers.map((offer, i) => (
                        <div key={i} className="flex items-start space-x-3">
                          ...
                          <span className="font-inter text-gray-700">{offer}</span>
                        </div>
                      ))}
                    </div>
                    <div className="mt-8 p-6 bg-luxury-gold/10 rounded-xl">
                      <p className="font-inter text-gray-800 mb-4">
                        <strong>Lưu ý:</strong> Các ưu đãi trên chỉ áp dụng khi mua xe thông qua Thật Nguyễn. 
